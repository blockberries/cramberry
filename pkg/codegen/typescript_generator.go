...
func (c *tsContext) tsWireTypeForType(t schema.TypeRef) string {
	...
	case *schema.NamedType:
		...
		// For simplicity, we check if it's likely an enum by checking schema
...
{{if generateMarshal}}
import { Writer, Reader, WireType } from 'cramberry';
...
export function encode{{tsMessageType $msg}}(writer: Writer, msg: {{tsMessageType $msg}}): void {
  ...
  let fieldCount = 0;
{{- range $msg.Fields}}
  if (msg.{{tsFieldName .}} !== undefined{{if not .Optional}} && msg.{{tsFieldName .}} !== null{{end}}) fieldCount++;
{{- end}}
