...
// Type indicates how a value is encoded on the wire.
type Type uint8
...
func NewTag(fieldNum int, wireType Type) Tag {
...
func AppendTag(buf []byte, fieldNum int, wireType Type) []byte {
...
func DecodeTag(data []byte) (fieldNum int, wireType Type, n int, err error) {
...
func DecodeTagRelaxed(data []byte) (fieldNum int, wireType Type, n int, err error) {
	tag, n, err := DecodeUvarint(data)
	if err != nil {
		return 0, 0, 0, err
	}
...
func PutTag(buf []byte, fieldNum int, wireType Type) int {
	tag := uint64(fieldNum)<<3 | uint64(wireType)
...
// TypeForKind returns the typical wire type for a Go kind.
// This is a helper for reflection-based encoding.
func TypeForKind(kind string) Type {
	switch kind {
